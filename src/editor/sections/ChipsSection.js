import '../items/ChipItem.js';

class HaRoomCardEditorChipsSection extends HTMLElement {
  constructor() {
    super();
  ...
  set hass(hass) {
    ...
    this.shadowRoot.querySelectorAll('ha-room-card-editor-chip-item').forEach(el => { el.hass = hass; });
  }

  _fire() {
    this.dispatchEvent(new CustomEvent('chips-changed', {
      detail: { chips: [...this._chips] },
      bubbles: true,
      composed: true,
    }));
  }
  ...
  _addChip() {
    this._chips = [...this._chips, { type: 'light', entity: '', name: '' }];
    this._expandedIndex = this._chips.length - 1;
    this._render();
    this._fire();
  }
  ...
  _removeChip(index) {
    ...
    if (this._expandedIndex >= this._chips.length) this._expandedIndex = null;
    this._render();
    this._fire();
  }

  _moveChip(oldIndex, newIndex) {
    const items = [...this._chips];
  ...
  _updateChip(index, config) {
    this._chips = this._chips.map((c, i) => i === index ? { ...config } : c);
    this._fire();
  }
  ...
  _label(chip) {
    const parts = [chip.type || '?'];
    if (chip.name) parts.push(chip.name);
    if (chip.entity) parts.push(chip.entity);
    return parts.join(' · ');
  }
  ...
  _render() {
    ...
    sortable.addEventListener('item-moved', e => {
      this._moveChip(e.detail.oldIndex, e.detail.newIndex);
    });
    ...
    this._chips.forEach((chip, index) => {
      ...
      labelEl.textContent = this._label(chip);
      ...
      deleteBtn.addEventListener('click', e => { e.stopPropagation(); this._removeChip(index); });
      ...
      if (isExpanded) {
        ...
        const chipItem = document.createElement('ha-room-card-editor-chip-item');
        chipItem.config = chip;
        chipItem.hass = this._hass;
        chipItem.addEventListener('config-changed', e => {
          this._updateChip(index, e.detail.config);
          labelEl.textContent = this._label(e.detail.config);
        });
        content.appendChild(chipItem);
    ...
    addBtn.innerHTML = '<ha-icon icon="mdi:plus-circle-outline"></ha-icon> Aggiungi chip';
    addBtn.addEventListener('click', () => this._addChip());
    root.appendChild(addBtn);
  }
}

customElements.define('ha-room-card-editor-chips', HaRoomCardEditorChipsSection);
